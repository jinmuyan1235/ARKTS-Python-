...
- 识别：从内置微型测试集自由点选任意图片，也可从图库选择，显示后台排队/推理状态和失败提示。
- 批量识别：逐项勾选或追加 8 张内置测试图，也可从图库多选最多 30 张；可查看全部已选图片、逐张删除和查看剩余容量，并显示总进度、分类统计和逐项结果。
- 后台恢复：单图、批量和文档任务标识保存在 HarmonyOS Preferences；回到应用或重启后继续查询，已上传文档也可恢复打开。
...
- 设置：所有账号都可配置、检测和保存电脑端服务连接；还可编辑显示名称和角色、更换头像、修改密码、查看组员、安全存储状态和退出登录。
