...
`D:\arkts_project` 提供 HarmonyOS ArkTS 手机端，包含单图识别、批量识别、SMILES、历史、设置、独立结果详情页、导出中心、反馈数据审核和文档区域审核工作台。电脑端使用带 API 密钥的本地 FastAPI 服务，图片、批量与文档任务以持久化后台任务运行。批量页支持多图上传、总进度、六类统计、逐项结果、暂停、继续、取消、失败重试，以及应用重启后恢复未完成任务。设置页模型状态完全来自真实健康检查；反馈审核复用 `src/feedback` 与训练 Manifest 门禁，未核验样本不能进入训练集。导出中心支持把单条或批量 CSV、JSON、PDF、SMI、MOL、SDF 和 ZIP 通过短时签名地址下载到系统文件或分享；正式结构格式只包含人工确认结果。

移动端还提供本地登录注册、可撤销会话和组员资料。密码只以 PBKDF2 加盐哈希保存在电脑端 SQLite；每个组员可以设置姓名、角色和独立头像。

启动、接口与模拟器说明见 [docs/harmony_mobile_api.md](docs/harmony_mobile_api.md)。
